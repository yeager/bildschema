...
from bildschema.accessibility import AccessibilityManager

TEXTDOMAIN = "bildschema"
gettext.textdomain(TEXTDOMAIN)
_ = gettext.gettext

APP_ID = "se.yeager.bildschema"
CONFIG_DIR = Path(GLib.get_user_config_dir()) / "bildschema"
...
class BildschemaApp(Adw.Application):
    ...
    def _on_preferences(self, *_args):
        ...
        cache_row.set_subtitle(f"{cache_mb:.1f} MB")
    ...
    def _on_export(self, *_args):
        ...
        if win and hasattr(win, 'activities'):
            from bildschema.export import show_export_dialog
            show_export_dialog(win, win.activities,
    ...
    def _on_about(self, *_args):
        about = Adw.AboutDialog(
            ...
            comments=_(
                "Visual daily schedules with pictogram support for "
                "children with autism and language disorders.\n\n"
                "Part of the Autismappar suite — free tools for "
                "communication and daily structure."
            ),
            debug_info=f"TTS: {__import__('bildschema.tts', fromlist=['get_tts_info']).get_tts_info()}\n"
                       f"Version: {__version__}\n"
                       f"GTK: {Gtk.get_major_version()}.{Gtk.get_minor_version()}\n"
                       f"Adwaita: {Adw.get_major_version()}.{Adw.get_minor_version()}\n"
                       f"Python: {sys.version}",
            debug_info_filename="bildschema-debug-info.txt",
        )
        ...
        about.add_link(_("Help translate"), "https://app.transifex.com/danielnylander/bildschema")
